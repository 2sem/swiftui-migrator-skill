...
> **Note on GADManager Initialization:**
> A common mistake is to try to initialize `GADManager` with test units, like so:
> ```swift
> // INCORRECT INITIALIZATION
> manager = GADManager<GADUnitName>(testingUnits: testUnits)
> ```
> The `GADManager` package does not have an initializer that accepts `testingUnits`. The correct way to initialize it is by passing the application's `UIWindow` instance. The `testUnits` array is used with the `isTesting` parameter in methods like `show()` and `prepare()`, not during initialization.
>
> The correct initialization, as shown in the sample file, is:
> ```swift
...
>         manager?.prepare(unit: interstitialUnit, interval: interval)
>     }
> ```
>
> **Correct:**
